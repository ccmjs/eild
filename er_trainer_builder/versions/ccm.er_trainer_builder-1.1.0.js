...
//    "data": { "store": [ "ccm.store" ] },
  ...
  //  "onfinish": { "restart": true },
    ...
    Instance: function () {
       ...
       */
      this.render = ( config = this.getValue() ) => this.html.render( this.html.main( config, this, onImportPhrases, onFileSelected, onExportPhrases, showPreview, onShowPhrase, onDeleteNotation, onResetNotations, onDeletePhrase ), this.element );
      ...
      /** imports phrases from a CSV file */
      const onImportPhrases = () => this.element.querySelector( 'input[type=file]' ).click();

      /** reads the CSV file and extracts contained phrases */
        ...
        reader.onload = () => {
          ...
          data = data.map( phrase => {
            ...
            return {
              key: $.generateKey(),
              text: phrase[ 0 ],
              relationship: [ phrase[ 1 ], phrase[ 2 ], phrase[ 3 ] ],
              solution: [ phrase[ 4 ], phrase[ 5 ] ],
              comment: [ phrase[ 6 ], phrase[ 7 ] ]
            };
          } );
          const phrases = {};
          data.forEach( phrase => phrase.text && ( phrases[ phrase.key ] = phrase ) );
      ...
      /** exports the phrases as CSV file */
      const onExportPhrases = () => {

        let rows = Object.values( this.getValue().phrases );
        rows = rows.map( phrase => [
          phrase.text,
        ...
        rows.unshift( [ 'PHRASE', 'ENTITY1', 'RELATION', 'ENTITY2', 'LEFT SOLUTION', 'RIGHT SOLUTION', 'LEFT COMMENT', 'RIGHT COMMENT' ] );
        window.open( encodeURI( 'data:text/csv;charset=utf-8,\uFEFF' + rows.map( row => row.join( ';' ) ).join( '\n' ) ) );
