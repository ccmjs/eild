    ...
    function connection( nr ) {
      return html`
        <div class="${ nr > 2 ? 'vertical' : '' }">
          <img class="${ nr === 1 && notation.left || '' }" src="${ notation.images[ app.values.indexOf( section[ is_solution ? 'solution' : 'input' ][ notation.swap ? ( nr > 1 ? 0 : 1 ) : nr - 1 ] ) + 1 ] }" ?data-hidden=${ !phrase.entities[ nr - 1 ] }>
        </div>
      `;
    }
