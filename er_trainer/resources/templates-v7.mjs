    ...
    function connection( nr ) {
      return nr ? html`
        <div class="${ nr > 2 ? 'vertical' : '' }">
          <img class="${ nr === 1 && notation.mirrored ? 'mirrored' : '' }" src="${ notation.images[ app.values[ 0 ].indexOf( ( is_solution ? phrase.solution : section.input )[ notation.swap ? ( nr > 1 ? 0 : 1 ) : nr - 1 ] ) + 1 ] }" ?data-hidden=${ !phrase.entities[ nr - 1 ] }>
        </div>
      ` : html`<div></div>`;
    }
